...
class GammaScalping:
    ...
    def getInitialBalance(self):
        ...
        call_premium = getOptionPrice(0, 'call')
        put_premium = getOptionPrice(0, 'put')
    ...
    def calcDelta(self, idx):
        ...
        call_delta = getDelta(spot_price, self.c_strike, self.c_expiry, self.rate, sigma, 'call')
        assert call_delta <= 1 and call_delta >= 0, 'Call delta not in range error'
        print("call delta : {}".format(call_delta)) # apply checks

        put_delta = getDelta(spot_price, self.p_strike, self.p_expiry, self.rate, sigma, 'put')
    ...
    def deltaHedge(self, idx):
        ...
        if delta > 0 + self.delta_tolerence: ## look at round off while hedging
